...
async function main() {
  ...
  const games = await Promise.all([
    ...
    prisma.game.create({ data: {
      gameType: 'POKEMON',
      description: 'Format Standard, decks construits. Niveau intermédiaire bienvenu, débutants OK si motivés !',
      address: '15 Rue de Bretagne, 75003 Paris',
      latitude: 48.8621, longitude: 2.3612,
      scheduledAt: inH(3),
      duration: 120, maxPlayers: 4,
      status: 'OPEN',
      creatorId: romain.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'POKEMON',
      description: 'Draft Pokémon ! On se partage les boosters, format libre ensuite.',
      address: 'Place de la République, 75011 Paris',
      latitude: 48.8674, longitude: 2.3633,
      scheduledAt: inD(2),
      duration: 180, maxPlayers: 4,
      status: 'OPEN',
      creatorId: romain.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'POKEMON',
      description: 'Mini tournoi 4 joueurs, round-robin. On se retrouve au café.',
      address: 'Rue Oberkampf, 75011 Paris',
      latitude: 48.8648, longitude: 2.3743,
      scheduledAt: inH(6),
      duration: 180, maxPlayers: 4,
      status: 'FULL',
      wasFilledOnce: true,
    ...
    prisma.game.create({ data: {
      ...
      latitude: 48.8533, longitude: 2.3692,
      scheduledAt: inH(5),
      duration: 90, maxPlayers: 2,
      status: 'OPEN',
      creatorId: dev.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'ONE_PIECE',
      description: 'Session One Piece TCG débutant/intermédiaire. Échange de cartes possible après !',
      address: '2 Place du Tertre, 75018 Paris',
      latitude: 48.8867, longitude: 2.3411,
      scheduledAt: inD(1),
      duration: 90, maxPlayers: 3,
      status: 'OPEN',
      creatorId: dev.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'NARUTO',
      description: 'Naruto TCG old school, j\'ai des decks de prêt si t\'as pas le tien.',
      address: 'Place de la Nation, 75012 Paris',
      latitude: 48.8484, longitude: 2.3960,
      scheduledAt: inH(8),
      duration: 90, maxPlayers: 2,
      status: 'OPEN',
      creatorId: theo.id,
    ...
    prisma.game.create({ data: {
      gameType: 'YUGIOH',
      description: 'Format Master Duel IRL. Deck combo bienvenu, niveau expert.',
      address: 'Place de l\'Opéra, 75009 Paris',
      latitude: 48.8719, longitude: 2.3316,
      scheduledAt: inH(10),
      duration: 120, maxPlayers: 2,
      status: 'OPEN',
      creatorId: lucas.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'ONE_PIECE',
      description: 'Tournoi 4 joueurs round-robin One Piece TCG. Set OP-07 autorisé.',
      address: '50 Rue de Belleville, 75020 Paris',
      latitude: 48.8725, longitude: 2.3783,
      scheduledAt: inD(3),
      duration: 240, maxPlayers: 4,
      status: 'FULL',
      wasFilledOnce: true,
      creatorId: emma.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'POKEMON',
      description: 'Format Expanded, niveau compétitif. Cherche joueur expérimenté.',
    ...
    prisma.game.create({ data: {
      gameType: 'NARUTO',
      description: 'Casual Naruto TCG, tous niveaux bienvenus. On boit un café en jouant.',
      address: 'Café de Flore, 75006 Paris',
      latitude: 48.8540, longitude: 2.3331,
      scheduledAt: inH(12),
      duration: 60, maxPlayers: 2,
      status: 'OPEN',
      creatorId: sakura.id,
    }}),
    ...
    prisma.game.create({ data: {
      gameType: 'YUGIOH',
      description: 'YGO débutant friendly, on peut expliquer les règles si besoin.',
      address: 'Forum des Halles, 75001 Paris',
      latitude: 48.8624, longitude: 2.3464,
      scheduledAt: inH(24),
      duration: 90, maxPlayers: 2,
      status: 'OPEN',
      creatorId: zack.id,
    ...
    prisma.game.create({ data: {
      ...
      latitude: 48.8584, longitude: 2.3470,
      scheduledAt: agoH(2),
      duration: 90, maxPlayers: 2,
      status: 'CANCELLED',
      creatorId: sakura.id,
  ...
  const [pG1Zack, pG1Sakura] = await Promise.all([
    prisma.participation.create({
      data: { userId: zack.id,   gameId: g1.id, status: 'ACCEPTED', acceptedAt: agoH(2) },
    }),
    prisma.participation.create({
  ...
  const [pG3Dev, pG3Lucas, pG3Emma] = await Promise.all([
    prisma.participation.create({
      data: { userId: dev.id,   gameId: g3.id, status: 'ACCEPTED', acceptedAt: agoH(8) },
    }),
    prisma.participation.create({
      data: { userId: lucas.id, gameId: g3.id, status: 'ACCEPTED', acceptedAt: agoH(7) },
    }),
    prisma.participation.create({
      data: { userId: emma.id,  gameId: g3.id, status: 'ACCEPTED', acceptedAt: agoH(6) },
    }),
  ]);
  ...
  const [pG5Romain, pG5Theo] = await Promise.all([
    prisma.participation.create({
      data: { userId: romain.id, gameId: g5.id, status: 'ACCEPTED', acceptedAt: agoH(12) },
    }),
    prisma.participation.create({
      data: { userId: theo.id,   gameId: g5.id, status: 'ACCEPTED', acceptedAt: agoH(11) },
    }),
  ]);
  ...
  await Promise.all([
    prisma.participation.create({
      data: { userId: romain.id, gameId: g8.id, status: 'ACCEPTED', acceptedAt: agoH(20) },
    }),
    prisma.participation.create({
      data: { userId: lucas.id,  gameId: g8.id, status: 'ACCEPTED', acceptedAt: agoH(18) },
    }),
    prisma.participation.create({
      data: { userId: zack.id,   gameId: g8.id, status: 'ACCEPTED', acceptedAt: agoH(15) },
    }),
  ]);
  ...
  await prisma.message.createMany({ data: [
    { gameId: g3.id, senderId: romain.id, content: 'Salut tout le monde ! On se retrouve au café Oberkampf, y\'a une grande table au fond.',            createdAt: agoH(7.5) },
    { gameId: g3.id, senderId: dev.id,    content: 'Parfait, j\'arrive vers 14h. J\'amène mon deck Charizard ex.',                                        createdAt: agoH(7.3) },
    { gameId: g3.id, senderId: lucas.id,  content: 'Okay pour moi aussi. Je joue un Blue-Eyes surprise 👁️',                                              createdAt: agoH(7.1) },
    { gameId: g3.id, senderId: romain.id, content: 'Haha la surprise on verra ça sur table. Emma tu joues quoi ?',                                        createdAt: agoH(7.0) },
    { gameId: g3.id, senderId: emma.id,   content: 'Un deck Luffy tcg pour changer un peu 😄 Ça compte pas hein c\'est pour rire',                        createdAt: agoH(6.8) },
    { gameId: g3.id, senderId: dev.id,    content: 'Lol, on accepte tout. Hâte de voir ça !',                                                             createdAt: agoH(6.5) },
    { gameId: g3.id, senderId: romain.id, content: 'Bon on fait ça en round-robin ? Chacun joue contre chacun, 2 sets par match.',                        createdAt: agoH(5.0) },
    { gameId: g3.id, senderId: dev.id,    content: 'Oui carrément. On note les scores ?',                                                                 createdAt: agoH(4.8) },
    { gameId: g3.id, senderId: lucas.id,  content: 'Je prends un tableau sur mon tel. Plus qu\'à venir 🎴',                                               createdAt: agoH(4.5) },
  ]});

  // G5 — Chat One Piece Montmartre : Dev + Romain + Theo
  await prisma.message.createMany({ data: [
    { gameId: g5.id, senderId: dev.id,    content: 'Hey ! Je suis le créateur, on se retrouve à la Place du Tertre, y\'a des tables dehors.',             createdAt: agoH(11.5) },
    { gameId: g5.id, senderId: romain.id, content: 'Nickel ! Je connais bien l\'endroit. Vers quelle heure exactement ?',                                 createdAt: agoH(11.2) },
    { gameId: g5.id, senderId: dev.id,    content: 'Demain 15h ça te va ? J\'ai précisé dans la partie mais au cas où.',                                  createdAt: agoH(11.0) },
    { gameId: g5.id, senderId: theo.id,   content: 'Moi je serai là. Je joue One Piece depuis peu donc soyez indulgents haha',                            createdAt: agoH(10.5) },
    { gameId: g5.id, senderId: romain.id, content: 'Pas de souci Theo ! On est là pour jouer et rigoler. Tu connais les bases au moins ?',                createdAt: agoH(10.2) },
    { gameId: g5.id, senderId: theo.id,   content: 'Oui oui les règles je les connais, juste que je maîtrise pas encore toutes les synergies.',           createdAt: agoH(10.0) },
    { gameId: g5.id, senderId: dev.id,    content: 'Super, à demain alors ! Je serai en noir, chapeau rouge 🎩',                                          createdAt: agoH(9.5) },
    { gameId: g5.id, senderId: romain.id, content: 'Haha parfait, facile à repérer. À demain !',                                                          createdAt: agoH(9.0) },
  ]});

  // G1 — Chat Pokémon Marais : Romain + Zack (ZackDuel est ACCEPTED)
  await prisma.message.createMany({ data: [
    { gameId: g1.id, senderId: zack.id,   content: 'Salut ! Je joue un deck Miraidon ex, ça te dérange pas ?',                                            createdAt: agoH(2.5) },
    { gameId: g1.id, senderId: romain.id, content: 'Pas du tout, j\'ai un Charizard ex. Ça va être un bon match !',                                       createdAt: agoH(2.3) },
    { gameId: g1.id, senderId: zack.id,   content: 'Top ! On se retrouve bien au café du coin de la rue ?',                                               createdAt: agoH(2.1) },
    { gameId: g1.id, senderId: romain.id, content: 'Ouais parfait, à tout à l\'heure 👍',                                                                 createdAt: agoH(2.0) },
  ]});
  ...
  console.log('  G2  Pokémon  OPEN   République     +48h  (par Romain) ← 2 demandes PENDING');
  console.log('  G3  Pokémon  FULL   Oberkampf      +6h   (par Romain) ← Dev ACCEPTED → chat actif');
  console.log('  G4  YGO      OPEN   Bastille       +5h   (par Dev)    ← Romain PENDING');
  console.log('  G5  OnePiece OPEN   Montmartre     +24h  (par Dev)    ← Romain ACCEPTED → chat actif');
  console.log('  G6  Naruto   OPEN   Nation         +8h   (par Theo)');
  console.log('  G7  YGO      OPEN   Opéra          +10h  (par Lucas)');
  console.log('  G8  OnePiece FULL   Belleville     +72h  (par Emma)   ← Romain ACCEPTED');
  ...
  console.log('  Romain : 4 non lues (2× demandes G2, 1× demande G1, 1× message G5)');
  console.log('  Dev    : 2 non lues (demande de Romain G4, message G3)\n');
  console.log('💬  CONVERSATIONS ACTIVES\n');
  console.log('  G3 Pokémon Oberkampf   → Romain + Dev + Lucas + Emma (9 messages)');
